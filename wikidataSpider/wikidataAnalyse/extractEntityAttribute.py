...
#加载实体列表
entitySet = set()
with open("new_node.csv",'r',encoding='utf-8') as fr:
	newNodeCsv = csv.reader(fr)
	for item in newNodeCsv:
		entitySet.add(item[0].strip())
with open("predict_labels_accurate.txt",'r',encoding='utf-8') as fr:
	for line in fr:
		predictLabel = line.split()[0]
		entitySet.add(predictLabel) 

with open('school_pedia1.csv','r',encoding='utf-8') as fr:
	with open('attributes1.csv','w',encoding='utf-8') as fw:
		fw.write('Entity'+','+'AttributeName'+','+'Attribute'+'\n')
		#从school_pedia.csv中提取出属性
