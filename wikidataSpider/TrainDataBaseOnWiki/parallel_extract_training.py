...
def get_NE(text):
    ...
    while i < length:
		p1 = TagList[i][0]
		p1 = p1.replace('(','（').replace(')','）').replace('*','').replace('-','').replace('+','').replace('^','').replace('/','').replace('%','').replace('=','').replace('~','').replace('<','').replace('>','').replace('{','').replace('}','').replace('[','').replace(']','')
		p1 = re.sub('\"','',p1).strip()
		t1 = TagList[i][1]
		p2 = TagList[i+1][0]
		t2 = TagList[i+1][1]
		p12 = p1 + TagList[i+1][0]
		p12 = p12.replace('(','（').replace(')','）').replace('*','').replace('-','').replace('+','').replace('^','').replace('/','').replace('%','').replace('=','').replace('~','').replace('<','').replace('>','').replace('{','').replace('}','').replace('[','').replace(']','')
...
corpus_path = os.path.abspath(os.path.join(os.getcwd(),"extracted/AA/"))
