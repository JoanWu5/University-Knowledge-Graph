...
class Neo4j():
	...
	def matchSchoolItembyTitle(self,value):
		...
		answer = matcher.match('SchoolItem').where('_.title =~'+'"'+str(value)+'"').first()
		return answer
	...
	def findRelationByEntities(self,entity1,entity2):
		answer = self.graph.run("MATCH (p1:SchoolItem {title:\"" + str(entity1) + "\"}),(p2:SchoolItem{title:\""+str(entity2)+"\"}),p=shortestpath((p1)-[rel:RELATION*]-(p2)) RETURN rel").evaluate()
		#answer = self.graph.run("MATCH (p1:SchoolItem {title:\"" + entity1 + "\"})-[rel:RELATION]-(p2:SchoolItem{title:\""+entity2+"\"}) RETURN p1,p2").data()
		
		if(answer is None):	
			answer = self.graph.run("MATCH (p1:SchoolItem {title:\"" + str(entity1) + "\"}),(p2:NewNode {title:\""+str(entity2)+"\"}),p=shortestpath((p1)-[rel:RELATION*]-(p2)) RETURN p").evaluate()
		if(answer is None):
			answer = self.graph.run("MATCH (p1:NewNode {title:\"" + str(entity1) + "\"}),(p2:SchoolItem{title:\""+str(entity2)+"\"}),p=shortestpath((p1)-[rel:RELATION*]-(p2)) RETURN p").evaluate()
		if(answer is None):
		...
		#answer = self.graph.data("MATCH (n1:SchoolItem {title:\"" + entity1 + "\"})- [rel] -> (n2:SchoolItem{title:\""+entity2+"\"}) RETURN n1,rel,n2" )
		#if(answer is None):
		#	answer = self.graph.data("MATCH (n1:SchoolItem {title:\"" + entity1 + "\"})- [rel] -> (n2:NewNode{title:\""+entity2+"\"}) RETURN n1,rel,n2" )
		#if(answer is None):
		#	answer = self.graph.data("MATCH (n1:NewNode {title:\"" + entity1 + "\"})- [rel] -> (n2:SchoolItem{title:\""+entity2+"\"}) RETURN n1,rel,n2" )
		#if(answer is None):
	...
	def findEntityRelation(self,entity1,relation,entity2):
		answer = self.graph.run("MATCH (n1:SchoolItem {title:\"" + str(entity1) + "\"})- [rel:RELATION {type:\""+str(relation)+"\"}] -> (n2:SchoolItem{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
		if(answer is None):
			answer = self.graph.run("MATCH (n1:SchoolItem {title:\"" + str(entity1) + "\"})- [rel:RELATION {type:\""+str(relation)+"\"}] -> (n2:NewNode{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
		if(answer is None):
			answer = self.graph.run("MATCH (n1:NewNode {title:\"" + str(entity1) + "\"})- [rel:RELATION {type:\""+str(relation)+"\"}] -> (n2:SchoolItem{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
		if(answer is None):
