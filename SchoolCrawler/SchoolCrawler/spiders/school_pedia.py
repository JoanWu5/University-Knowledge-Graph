...
class SchoolSpider(scrapy.Spider):
	name = "school"   #爬虫启动命令：scrapy crawl school
...
#	start_urls.append('http://www.baike.com/wiki/李%5B蔷薇科李属植物%5D')
  ...
  # 本处是用于构造原始json
	...
	def parse(self, response):		
		...
		img = ""   # 爬取图片url
		for p in response.xpath('//div[@class="w-990"]/div[@class="r w-300"]/div[@class="doc-img"]/a/img/@src'):
			img = p.extract().strip()
				
		openTypeList = ""  # 爬取开放域标签
		flag = 0   #flag用于分隔符处理（第一个词前面不插入分隔符）
		for p in response.xpath('//div[@class="w-990"]/div[@class="l w-640"]/div[@class="place"]/p[@id="openCatp"]/a/@title'):
			if flag == 1 :
		...
		detail = "" # 详细信息
		detail_xpath = response.xpath('//div[@class="w-990"]/div[@class="l w-640"]/div[@class="information"]/div[@class="summary"]/p')
		if len(detail_xpath) > 0 :   
			detail = detail_xpath.xpath('string(.)').extract()[0].strip()
	
		if detail == "":  # 可能没有
			detail_xpath = response.xpath('//div[@class="w-990"]/div[@class="l w-640"]/div[@id="content"]')
			if len(detail_xpath) > 0 :   
				detail = detail_xpath.xpath('string(.)').extract()[0].strip()
		
		flag = 0
		baseInfoKeyList = "" #基本信息的key值
		for p in response.xpath('//div[@class="w-990"]/div[@class="l w-640"]/div[@name="datamodule"]/div[@class="module zoom"]/table//strong/text()'):  
			if flag == 1 :
		...
		base_xpath = response.xpath('//div[@class="w-990"]/div[@class="l w-640"]/div[@name="datamodule"]/div[@class="module zoom"]/table')
		for p in base_xpath.xpath('.//span'):
			...
			flag = 1
		# print(title,url,img,openTypeList,baseInfoKeyList,baseInfoValueList)
